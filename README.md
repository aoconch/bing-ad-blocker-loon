# Bing AD Blocker for Loon

屏蔽 **Microsoft Bing App** 中的广告与推广内容。基于 **2026-08-31 iPhone 真机 HAR 抓包** 校准，去广告规则对准真实流量，而非猜测。

**v8 起改为「纯 JS、零 Rule」**：不再依赖 Loon 的 `[Rule]` 域名拦截，所有去广告逻辑都在两个 JS 脚本里完成。

**v15**（HAR #297）：`rewards.bing.com` 完全旁路——MitM 去掉 `*.bing.com`、排除项置前；搜索脚本改为子域白名单；不再处理 `rewardsplatform`。
**v14**：尝试排除 `rewards.bing.com`（真机仍命中脚本，已由 v15 加固）。
**v13**：请求脚本只匹配广告域，避免登录/认证 Script timeout。

## 原理（两个脚本各司其职）

| 阶段 | 脚本 | 干的事 | 替代了什么 |
|------|------|--------|-----------|
| 请求前 | `bing_block_request.js` (http-request) | 按域名后缀拦截广告 / RTB 竞价 / 推广 / 追踪请求，返回空响应 | 原 `[Rule]` 全部 `DOMAIN-SUFFIX,REJECT` |
| 响应后 | `bing_remove_ads.js` (http-response) | 递归剔除混在合法响应里的内联广告（首页信息流 `nativead` 卡、搜索结果推广、文章植入） | 原脚本的 `[Script]` 内联剥离 |

为什么还要脚本剥离内联广告：像首页信息流、搜索结果页的广告是 **Bing 自己的接口返回的数据**（嵌在 `assets.msn.com` / `assets.msn.cn` / `bing.com` 的 JSON 或 HTML 里），光拦截域名拦不到——必须解析响应、把广告对象/容器删掉。而独立的广告网络请求（RTB、doubleclick、追踪 SDK）则由请求脚本直接掐断。

## 文件

- `Bing-AD-Blocker.plugin` — Loon 插件（`[Mitm]` + `[Script]`，**无 `[Rule]`**）
- `bing_block_request.js` — 请求拦截脚本：域名后缀表 + 返回空响应
- `bing_remove_ads.js` — 响应剥离脚本：递归移除 `nativead` 原生广告卡与带 Ad 角标的区域

## 安装（Loon 一键导入）

**推荐：commit 固定地址（不走 CDN 缓存，永远拿到确切版本）**

```
https://cdn.jsdelivr.net/gh/aoconch/bing-ad-blocker-loon@167f61e/Bing-AD-Blocker.plugin
```

Loon 操作：配置 → 插件 → `+` → 通过 URL 添加 → 粘贴上面地址 → 启用。
若之前装过旧版，先左滑删除旧插件再添加（Loon 插件缓存按 URL 区分，新 URL = 重新拉取）。

备用地址（raw.githubusercontent，需要代理可直连 GitHub）：
`https://raw.githubusercontent.com/aoconch/bing-ad-blocker-loon/main/Bing-AD-Blocker.plugin`

> ⚠️ 不要用 `@main` 的 jsDelivr 地址：jsDelivr 对 `@main` 有约 45 分钟的更新节流。

安装后：Loon → 工具 → MitM 开启 → 安装并信任证书 → 打开 Bing App。
**不要**对 `login.microsoftonline.com` / `odc.officeapps.live.com` / `bingapiauth.*` 做 MitM。
**`rewards.bing.com` / `prod.rewardsplatform.microsoft.com` 完全不 MitM、不跑脚本。**

## 手动部署到自己的仓库

1. 把本仓库推到 GitHub（任意公开仓库）。
2. 编辑 `Bing-AD-Blocker.plugin`，把脚本地址换成你自己的。
3. Loon 通过 URL 添加：`https://cdn.jsdelivr.net/gh/<你>/<仓库>@167f61e/Bing-AD-Blocker.plugin`

> **Bing 奖励默认保留**：Rewards 相关域名全部旁路，积分与日常任务原样保留。

## 怎么确认插件真的生效了

- **请求拦截**：被拦请求带 `X-Loon-AdBlock-Req: blocked=1`。
- **内联剥离**：响应头 **`X-Loon-AdBlock: removed=N;v=15`**。
- **Rewards 旁路**：`rewards.bing.com` 的 Loon 记录里 **`script` 应为空**，且 **`mitmHost` 不应为 rewards.bing.com**。

## 故障排查

### 症状：rewards.bing.com 无法浏览（HAR #297）

抓包若仍见 `_loon.script: ["Bing去广告(搜索)"]` 且 `mitmHost: rewards.bing.com`，说明还在跑旧插件（`*.bing.com` 通配）。

**处理**：删除全部旧 Bing 去广告插件 → 用 v15 commit 固定 URL 重装 → 强杀 Bing/Edge 再开。

### 症状：登录 / Rewards API 不可用

旧版全量 `https://` 请求脚本超时。装 v13+；确认登录域无请求脚本。

### 症状：有广告但版本不对

响应头不是 `v=15` → 换新 commit URL 重装。
